...
\begin{figure}[ht]
  ...
  \includegraphics[width=14cm]{3dfaster_rcnn_frame}
...
\begin{figure}[ht]
  \centering
  % @improve: more specific
  \includegraphics[width=14cm]{faster_rcnn_frame}
...
Mask R-CNN相比Faster R-CNN不仅可以输出目标的Class和Bounding Box，还可以输出目标的Mask。Mask R-CNN通过对网络的RoI单独增加一个分支来生成目标的Mask，
\section{目标检测实验}
...
\subsection{实验结果}
在APC数据集上，本文算法Faster R-CNN和Mask R-CNN的精确度如表\ref{tab:ap1}所示。
\begin{table}[ht]
  \centering
  ...
  \label{tab:ap1}
\end{table}
从表\ref{tab:ap1}中可以看出在APC数据集上3D Faster R-CNN相比Faster R-CNN的精确度提高了1.3个百分点左右，3D Mask R-CNN相比Mask R-CNN提高了0.8个百分点左右。整体来说对精确度的提高并不是十分明显。

在workpiece数据集上，本文算法Faster R-CNN和Mask R-CNN的精确度如表\ref{tab:ap2}所示。
\begin{table}[ht]
  \centering
  ...
  \label{tab:ap2}
\end{table}
从表\ref{tab:ap2}可以看出在workpiece数据集上，3D Faster R-CNN相比Faster R-CNN的精确度提高了13.6个百分点左右，3D Mask R-CNN相比Mask R-CNN提高了约14.8个百分点。
